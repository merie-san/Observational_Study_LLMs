...
Our objective is to thoroughly analyze the usage and prominence of LLMs in GitHub repositories. Many different languages can be used to do this analysis, but we decided to only consider the following languages:
	...
	To reach our goal, we first analyzed for each language the evolution of the percentage of repositories containing LLM-adjacent words in their topic, description, or title. From the data we collected, we performed several analyses, such as which language is the most popular for the development of LLM-integrated applications or which repository metrics are more correlated. Starting from this analysis, we conducted a deeper research on public repositories on GitHub by using code search to retrieve all instances of relevant SDK import or model reference. The collected data is combined to obtain SDK-model pairs that can be analyzed and used to derive insights for our study. We then sampled a set of representative repositories from this exhaustive list to apply statistical methods and infer properties of the sampled population.\\
	Our empirical analysis leads to several key findings:
	\begin{itemize}
		\item \textbf{Rapid growth of LLM-related projects}, since the introduction of LLMs to public usage in 2022 the proportion of LLM-related repositories among the most-starred projects increased significantly across all languages, with \textit{Python} showing the steepest rise, from a 30\% to a stable 80\% reached in 2023. The other languages also follow a similar upward trend but not as steep.
		...
		\item \textbf{OpenAI and Google SDKs are the most adopted among official libraries}, although third-party libraries remain popular across all languages. For \textit{Python}, official SDKs show much stronger adoption than in \textit{Java} and \textit{Go}, where third-party libraries remains the most utilized.
		\item \textbf{A small set of LLM models accounts for most real-world usage}, Across languages, the top 20\% most-frequent models constitute over 80\% of model references, with \textit{OpenAI} models dominating and \textit{Google} models appearing as secondary but significant contributors.
		\item \textbf{Common usage patterns align with conversational, generative, and agentic workflows}, Based on keyword-based classification, LLMs in public repositories are primarily used for conversational agents, content generation, and multi-step agentic orchestration, with fewer repositories employing retrieval-augmented pipelines or domain-specific applications.
		\item \textbf{Repository-level attributes show interesting correlations}, Spearman correlation analyses reveal distinct patterns across languages. In \textit{Python}, open issues correlate strongly with fork counts, while in \textit{Java} and \textbf{Go} stars correlate more with open issues. Statistical hypothesis testing further shows that \textit{Python} LLM repositories have significantly fewer stars on average than \textit{Java} and \textit{Go} repositories (p < 0.05 in both one-tailed Welch t-tests), suggesting lower visibility or more experimental project character.
	\end{itemize}
	...
	\end{figure}
	In this first graph we can observe that the proportion of LLM-related terms in the most starred public GitHub Python projects, started from a level of 30\% in 2022 and increased until a stable 80\% in the first half of 2023. We can see that a majority of the most popular new Python projects were about AI and LLMs in the last three years.
	\begin{figure}[h!]
	...
	\begin{figure}[h!]
		\includegraphics[width=1\linewidth]{Figures/csharp\_llm\_fraction.png}
		\caption{monthly proportion of Public C\# projects with LLM-related terms in their description, name or topics}
		\label{fig:csharp_monthly}
	\end{figure}

	For Java and C\# we observe a lower incidence of LLM-related terms appearing in the most starred recent projects, however we can still see an evident increase in popularity in the last two years.
	\begin{figure}[h!]
	...
	\begin{figure}[h!]
		\includegraphics[width=1\linewidth]{Figures/javascript\_llm\_fraction.png}
		\caption{monthly proportion of Public Javascript projects with LLM-related terms in their description, name or topics}
		\label{fig:javascript_monthly}
	\end{figure}
	For Golang and Javascript GitHub projects we see higher proportions of LLM-related projects and a similar peak in the may of 2025.
	...
	\begin{enumerate}
		\item Conversational Agents --- Used for direct human-LLM interaction in dialog form (e.g., chatbots, virtual assistants, helpdesk systems).
		\item Generation Agents --- Used to produce text, code, or other content using LLMs (e.g., article writing, code completion, story generation).
		\item Retrieval-Augmented --- Uses external knowledge sources to enhance LLM outputs and improve correctness (e.g., document question answering, knowledge-base assisted generation).
		\item Agentic Orchestration --- Used when the LLM drives multi-step actions, calls APIs, or interacts with other software components (e.g., workflow automation, multi-step task agents, Auto-GPT style agents).
		\item Multimodal --- Handles multiple data types, such as text, images, audio, and video (e.g., image captioning, audio transcription, video summarization).
		\item Domain-Specific --- Tailored LLM applications designed for a particular field or domain (e.g., legal AI assistants, medical Q\&A systems, financial analysis tools).
	\end{enumerate}
	...
	We can observe from the bar charts we built that between the classified repositories most llm are used as conversational agents, for generation tasks or for agentic orchestration.
	...
	The Spearman's correlation matrices for the Java and Golang datasets are very similar with each-other, while the one built from the Python dataset is quite different. The biggest observable difference is that for python projects the number of open issues correlates the most with the network count (number of forks) while in the other two languages it correlates the most with the number of stars of a project. This suggests that Python LLM projects are less visible, with activity driven by contributor engagement rather than popularity. This may be due to Python’s low barrier to entry, leading to many experimental projects with few stars. Let us Hypothesis test this.\\
	...
	\end{itemize}
	Second test (Python vs Golang): 
	\begin{itemize}
		\item $H_0$ --- Null Hypothesis\\
		The mean number of stars in Python LLM Repositories is equal to the mean number of stars in Golang LLM Repositories.
		\item $H_1$ --- Alternative Hypothesis\\
		The mean number of stars in Python LLM Repositories is less than the mean number of stars in Golang LLM Repositories.
	\end{itemize}
	We randomly sampled 500 repositories for all three languages and took record of the star number, and the size for each repository, then we use the Anderson-Darling Test to evaluate if the size and stars data of the repositories are normally distributed. This is a fundamental phase because it will determine if we need to normalize the data to obtain an accurate statistical analysis. 
	The results, visible in Figures [n. delle figure], brings us to reject ne \textit{null hypotesis} since the $A^{2}$ values are far greater than the Significance Level choosen. And so there's the need to apply a normalization function to the data.
	\pagebreak
	
	\begin{figure}[h!]
		\includegraphics[width=1\linewidth]{Figures/anderson_darling_size_test.png}
		\caption{Results for the Anderson-Darling test for the size data.}
		\label{fig:anderson_size}
	\end{figure}
	\begin{figure}[h!]
		\includegraphics[width=1\linewidth]{Figures/anderson_darling_stars_test.png}
		\caption{Results for the Anderson-Darling test for the stars data.}
		\label{fig:anderson_stars}
	\end{figure}
	
	We choose the Box-Cox transformation after shifting the number of stars by one to avoid zeros, to normalize the data, to stabilize variance and make the distribution more Gaussian and suitable for t-tests, transforming the power law like distributions into more normal shapes. We then apply Welch's t-test (one-tailed) to compute the t-statistics and the p-value for both Hypothesis tests assuming a significance level of $\alpha=0.05$. One sided tests are reasonable here because we suppose that the number of stars is lower in python LLM projects.
	Sample statistics before Box-Cox
	\begin{itemize}
		\item $\overline{X}_{Python}=18.99$
		\item $S^2_{Python}=270.93^2$
		\item $\overline{X}_{Java}=25.34$
		\item $S^2_{Java}=179.75^2$
		\item $\overline{X}_{Go}=188.41$
		\item $S^2_{Go}=1986.66^2$
	\end{itemize}
	Sample statistics after Box-Cox:
	\begin{itemize}
		\item $\overline{X'}_{Python}=0.173$
		\item $S'^2_{Python}=0.268^2$
		\item $\overline{X'}_{Java}=0.245$
		\item $S'^2_{Java}=0.352^2$
		\item $\overline{X'}_{Go}=0.393$
		\item $S'^2_{Go}=0.506^2$
	\end{itemize}
	Lambda parameter for Box-Cox transformation:
	\begin{itemize}
		\item $\lambda_{Python}=-0.7517$
		\item $\lambda_{Java}=-0.6540$
		\item $\lambda_{Go}=-0.4958$
	\end{itemize}
	For different values of lambda the Box-Cox transformation applies different operations on the dataset. When $\lambda=1$ then we have the original variable, when $\lambda=0$ , we have the logarithm transformation, when $\lambda=-1$  we have the reciprocal transformation, and when $\lambda=0.5$  we have the square root. Our $\lambda\approx–0.7$ indicates a transformation close to reciprocal-root behavior
	...
	\begin{itemize}
		...
		$p=0.000000$
	\end{itemize}
	Both values are lower than the significance level $\alpha=0.05$ leading us to reject the null hypotheses $H_0$ for both comparisons. In other words, there is enough evidence to support that the mean number of stars for Python LLM projects are lower than those of Java and Golang LLM projects. Thus, Python LLM repositories have significantly fewer stars, supporting the observation of lower visibility compared to Java and Golang projects. A natural thing we can derive is that since Python repositories are generally less visible and more experimental, their size may be smaller than their corresponding Java and Golang projects. The paper \autocite{zhang2009distributionprogramsizesimplications} supports a log-normal distribution of program sizes so we can use the same procedure we applied for the number of stars.
	...
	Sample statistics before Box-Cox:
	\begin{itemize}
		\item $\overline{X}_{Python}=24.19$
		\item $S^2_{Python}=75.03^2$
		\item $\overline{X}_{Java}=49.25$
		\item $S^2_{Java}=356.95^2$
		\item $\overline{X}_{Go}=31.98$
		\item $S^2_{Go}=169.90^2$
	\end{itemize}
	Sample statistics after Box-Cox:
	\begin{itemize}
		\item $\overline{X'}_{Python}=0.643$
		\item $S'^2_{Python}=0.648^2$
		\item $\overline{X'}_{Java}=0.900$
		\item $S'^2_{Java}=0.817^2$
		\item $\overline{X'}_{Go}=0.728$
		\item $S'^2_{Go}=0.678^2$
	\end{itemize}
	Lambda parameter for Box-Cox transformation:
	\begin{itemize}
		\item $\lambda_{Python}=-0.5846$
		\item $\lambda_{Java}=-0.3503$
		\item $\lambda_{Go}=-0.4997$
	\end{itemize}
	T-statistic and p-value for the two Welch t-tests:
	\begin{itemize}
		\item Python vs Java ---\\
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Our analysis focuses exclusively on public GitHub repositories and three programming languages: Python, Java, and Go.
		...
		\begin{itemize}
			...
			\item While Box-Cox transformations help normalize skewed data, extreme values in popularity or size could influence results.
	...
	\begin{enumerate}
		\item Quantified the evolution of LLM-integrated software popularity over 2022–2025, showing rapid adoption, especially in Python.
		\item Identified the most popular SDKs (OpenAI, Google) and models, highlighting the 80/20 pattern where a few models dominate usage.
