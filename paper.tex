...
Our objective is to thoroughly analyze the usage and prominence of LLMs in GitHub repositories. Many different languages can be used to do this analysis, but we decided to only consider the following languages:
	...
	The Mann-Whitney U test is a non parametric test that tests whether the distributions underlying the two input samples are the same. In out specific case we apply the one-sided test checking that the distribution of star numbers in Python repositories are stochastically less than in other languages. Stochastically less meaning that x tend to be smaller than values from y in a distribution-wide sense. Formally X is stochastically less than Y means that:
	$P(X>u)\le P(Y>u)$ \hspace*{0.2cm} $\forall u$.\\
	The results were the following, considering a significance level of $\alpha=0.05$:
	\begin{itemize}
		\item Python vs Java ---\\
	...
	\begin{itemize}
		...
		The size of Python LLM Repositories has the same distribution Java LLM Repositories.
