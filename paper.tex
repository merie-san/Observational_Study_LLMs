...
Our objective is to thoroughly analyze the usage and prominence of LLMs in GitHub repositories. Many different languages can be used to do this analysis, but we decided to only consider the following languages:
	...
	\begin{itemize}
		...
		\item \textbf{Repository-level attributes show interesting correlations}, Spearman correlation analyses reveal distinct patterns across languages. In \textit{Python}, open issues correlate strongly with fork counts (Figure \textbf{\ref{fig:python_corr}}), while in \textit{Java} and \textbf{Go} stars correlate more with open issues (Figures \textbf{\ref{fig:java_corr} \ref{fig:go_corr}}). Statistical hypothesis testing further shows that \textit{Python} LLM repositories have significantly fewer stars on average than \textit{Java} and \textit{Go} repositories, suggesting lower visibility or more experimental project character.
	\end{itemize}
