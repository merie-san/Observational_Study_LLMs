...
Our objective is to thoroughly analyze the usage and prominence of LLMs in GitHub repositories. Many different languages can be used to do this analysis, but we decided to only consider the following languages:
	...
	\end{enumerate}
	We based our classification method by matching the description, topics and name of the repository against a list of probable keywords for each category. This keyword matching method is not very efficient and leaves a lot of the repositories unclassified.
	...
	Basically, the Mann-Whitney U test checks whether the distributions underlying the two input samples are the same. In our specific case we apply the one-sided test to check whether the distribution of star numbers in Python repositories is stochastically less than compared to the other two languages. Stochastically less means that values from X tend to be smaller than values from Y in a distribution-wide sense. Formally X is stochastically less than Y means that:
	...
	\begin{itemize}
		...
		The distribution of sizes in Python repositories is stochastically less than in Java repositories
	\end{itemize}
	...
	\begin{itemize}
		...
		\item $H_1$ --- Alternative Hypothesis\\
		The distribution of sizes in Python repositories is stochastically less than in Golang repositories
	\end{itemize}
